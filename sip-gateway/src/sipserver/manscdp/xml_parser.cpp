...
int CXmlParser::Load(const char *p_data, int p_size, tinyxml_doc_t &doc)
{
    ...
    if (doc.Error()) {
        LOG_ERROR("Load xml failed, error: {}", doc.ErrorID());
        return -1;
    }
    tinyxml_ele_t *ele = doc.RootElement();
    if (nullptr == ele || ele->Value() == nullptr) {
        LOG_ERROR("Load xml failed, error: root element does not exist.");
        return -2;
    }
    return 0;
}
...
int CXmlParser::ParseXmlHeader(tinyxml_doc_t &doc, manscdp_msgbody_header_t &bodyheader)
{
    ...
    LOG_DEBUG("[CXmlParser] ParseXmlHeader, category: {}, cmdtype: {}, devid: {}, sn: {}", 
        category_name
        , cmdtype_name
        , bodyheader.devid
        , bodyheader.sn);
...
int CXmlParser::parse_cmdcategory(tinyxml_ele_t *ele, manscdp_cmd_category_e &category, string &name)
{
    ...
    if (nullptr == ele_text) {
        LOG_ERROR("[CXmlMsgParser] parse_cmdcategory failed, error: root element does not have value");
        return -1;
    }
...
int CXmlParser::ParseEleStr(tinyxml_ele_t *ele, const char *name, string &textstr, bool brequired, bool bto_utf8)
{
    ...
    if (nullptr == child_ele) {
        if (brequired) {
            LOG_ERROR("ParseEleStr: {} does not exist.", name);
        }
    ...
    if (nullptr == ele_text) {
        if (brequired) {
            LOG_ERROR("ParseEleStr: {} does not have value.", name);
        }
        return brequired ? -1 : 0;
    }
    if (bto_utf8) {
...
int CXmlParser::ParseEleInt(tinyxml_ele_t *ele, const char *name, int &value, bool brequired)
{
    ...
    if (nullptr == child_ele) {
        if (brequired) {
            LOG_ERROR("parse_ele_int: {} does not exist.", name);
        }
        return brequired ? -1 : 0;
    }
    int ele_value;
    if (child_ele->QueryIntText(&ele_value) != tinyxml2::XML_SUCCESS) {
        if (brequired) {
            LOG_ERROR("parse_ele_int: {} has invalid value.", name);
        }
        return brequired ? -1 : 0;
    }
    value = ele_value;
